...
class CollectionRepositoryMock implements CollectionRepository {
  ...
  @override
  Future<Either<Failure, FullItem>> readFullItem(ItemId itemId) async {
    Item? item = _itemCollection[itemId.value];
    if (item == null) {
      try {
        final itemModel = await bggDataSource.getItem(id: itemId.value);
        FullItem fullItem = modelToFullItem(itemModel: itemModel);
        return Future.value(Right(fullItem));
  ...
  FullItem modelToFullItem({required ItemModel itemModel}) {
    return FullItem(
      id: ItemId.fromUniqueString(itemModel.id),
      title: itemModel.title,
      longDescription: itemModel.description,
